...
if($count>0){
    echo "<div class='alert alert-warning'>There is already as user registered with that email. Please choose another one.</div>"; exit;
}
...
if($count == 1){
    ...
    echo "<div class='alert alert-warning'>There was an error retrieving the email from the database.</div>";exit;   
}
...
$result = mysqli_query($link, $sql);
if(!$result){
    echo "<div class='alert alert-warning'>There was an error inserting the user details in the database.</div>";exit;
}else{
...
if(mail($newemail, 'Email Update for you Online Notes App', $message, 'From:'.'boofcourse@gmail.com')){
