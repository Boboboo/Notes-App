...
if(empty($_POST["loginemail"])){
    ...
    $email = filter_var($_POST["loginemail"], FILTER_SANITIZE_EMAIL);   //这里已经不需要validate email，sign up的时候已经validate了
}
...
$password = hash('sha256', $password);
        //Run query: Check combinaton of email & password exists，用email和password进行查询
...
if($count !== 1){
    ...
    if(empty($_POST['rememberme'])){
        ...
        $authentificator1 = bin2hex(openssl_random_pseudo_bytes(10));   //生成一个长度为10的随机字符串作为密码
        $authentificator2 = openssl_random_pseudo_bytes(20);            //用来生成指定个数的随机字节
        ...
        setcookie(
            "rememberme",       //name of the cookie
            $cookieValue,       //value of the cookie
            time() + 1296000    //expiration date，例如15天，15*24*60*60
        );
